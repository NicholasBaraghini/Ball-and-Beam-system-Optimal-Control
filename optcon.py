...
def DDP_comp_t_k(kk, xx, uu, xx_ref, uu_ref, params):
    ...
    for tt in range(TT - 1, 0, -1):
        ...
        KK_dir_term = stC['lux'] + dot3(dyn['fu'].T, PP[:, :, tt + 1], dyn['fx']) + dyn[
        ...
        # PP update
        PP_1_term = stC['lxx'] + dot3(dyn['fx'].T, PP[:, :, tt + 1], dyn['fx']) + dyn[
...
def Trajectory_Update(kk, xx, uu, xx_ref, uu_ref, xx_init, TT, cost_kk, gamma, Sigma, KK, pp,  params):
  ...
  for tt in range(0,TT):
    ...
    uu[:,tt,kk+1] = uu[:,tt,kk] + gamma*Sigma[:,tt] + np.matmul( KK[:,:,tt], (xx[:,tt,kk+1]-xx[:,tt,kk]) );

    # State vector update at time t
    xx[:,tt+1, kk+1] = BB_Dynamics(xx[:,tt,kk+1], uu[:,tt,kk+1], pp[:,tt+1], params);

    # Cost Function Increment contribution at time t
    cost[kk+1] = cost[kk+1] + Stage_Cost(xx[:,tt,kk+1], uu[:,tt,kk+1], xx_ref[:,tt], uu_ref[:,tt], params)['cost_t'];

  cost[kk+1] = cost[kk+1] + Terminal_Cost(xx[:,TT,kk+1], xx_ref[:,TT], params)['cost_T'];
